...
namespace WebAPI.Tests.Unit.Controllers
{
    public class AuthControllerTests
    {
        ...
        public void GoogleLogin_ReturnsChallengeResult()
        {
            ...
            challengeResult!.AuthenticationSchemes.Should().Contain("Google");
            challengeResult.Properties.Should().NotBeNull();
            challengeResult.Properties!.RedirectUri.Should().Be("https://localhost:5001/api/auth/google/response");
        }
        ...
        {
            // Arrange - Mock the authentication result for Google OAuth
            ...
            }).AddCookie("Test").AddGoogle("Google", options => { });

            var serviceProvider = services.BuildServiceProvider();
            var authService = serviceProvider.GetRequiredService<Microsoft.AspNetCore.Authentication.IAuthenticationService>();
            var authResult = Microsoft.AspNetCore.Authentication.AuthenticateResult.Success(
                new Microsoft.AspNetCore.Authentication.AuthenticationTicket(principal, "Google"));
            ...
            _userServiceMock.Setup(s => s.GetByEmail("newuser@example.com")).Returns((User)null);
            var newUser = new User
            {
                UserId = 2,
                Username = "newuser@example.com".Split('@')[0],
            ...
            _userServiceMock.Setup(s => s.Register(It.IsAny<RegisterRequestDTO>())).Returns(newUser);
            _userServiceMock.Setup(s => s.GetByEmail("newuser@example.com")).Returns(newUser);

            // Act
            var result = _controller.GoogleResponse();

            // Assert
            result.Should().BeOfType<BadRequestObjectResult>();  // This will fail initially, we need to properly mock the AuthenticateAsync
            var redirectResult = result as BadRequestObjectResult;
